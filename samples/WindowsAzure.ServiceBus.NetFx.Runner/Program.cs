...
class Program
{
    ...
    private static NamespaceManager SetupNamespaceManager(ServiceBusOptions options)
    {
        if (ServiceBusOptions == null)
            throw new Exception("Configuration not bound to ServiceBusOptions");

        var scheme = "sb";
        var tokenProvider = TokenProvider.CreateSharedAccessSignatureTokenProvider(ServiceBusOptions.ServiceBusKeyName, ServiceBusOptions.ServiceBusKey);
